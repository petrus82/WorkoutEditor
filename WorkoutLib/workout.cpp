...
[[nodiscard]] float Interval::writeToFile(float startTime)
{
  ...
  case WorkoutType::PercentMaxHeartRate:
    ...
    break;
  default:
    throw std::runtime_error("No implementation for type " + intervalTypeToString(m_type) + "in FitInterval.");
  }
  // Return startTime of the next interval which is current startTime + Duration of current interval
  float endTime{m_startTime + (float)m_duration.Minutes + (float)m_duration.Seconds * 1 / 60};
  return endTime;
}
