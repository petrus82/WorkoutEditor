...
void FitInterval::writeCommon()
{
  ...
  m_workoutStepMesg->SetDurationValue((m_duration.Minutes * 60 + m_duration.Seconds) * 1000);
}

void FitInterval::writeAbsoluteWatt()
{
  m_workoutStepMesg->SetTargetType(FIT_WKT_STEP_TARGET_POWER);
  if (m_value.To > 0)
  {
    // Power range
    m_workoutStepMesg->SetCustomTargetPowerLow(m_value.From + AbsolutePowerOffset);
    m_workoutStepMesg->SetCustomTargetPowerHigh(m_value.To + AbsolutePowerOffset);
  }
  else
  {
    // Power zone
...
void FitInterval::writePercentFTP()
{
  m_workoutStepMesg->SetTargetType(FIT_WKT_STEP_TARGET_POWER);
  if (m_value.To > 0)
  {
...
void FitInterval::writeAbsoluteHeartRate()
{
  m_workoutStepMesg->SetTargetType(FIT_WKT_STEP_TARGET_HEART_RATE);
  if (m_value.To > 0)
  {
    // heart rate range
    m_workoutStepMesg->SetCustomTargetHeartRateLow(m_value.From + AbsoluteHeartRateOffset);
    m_workoutStepMesg->SetCustomTargetHeartRateHigh(m_value.To + AbsoluteHeartRateOffset);
  }
  else
  {
    // heart rate zone
...
void FitInterval::writePercentMaxHeartRate()
{
  m_workoutStepMesg->SetTargetType(FIT_WKT_STEP_TARGET_HEART_RATE);
  if (m_value.To > 0)
  {
...
void FitWorkout::writeWorkout()
{
  ...
  workoutMsg.SetWktName(std::wstring(m_workoutName.begin(), m_workoutName.end()));
